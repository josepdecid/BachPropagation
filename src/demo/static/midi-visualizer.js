...
// This is needed so that the Shady DOM polyfill scopes the styles correctly.
if (window.ShadyCSS) window.ShadyCSS.prepareTemplate(templ, 'midi-visualizer');
...
class MidiVisualizer extends HTMLElement {
    constructor() {
        ...
        this._player.callbackObject = {
            ...
            run: (note) => {
                ...
                console.log(containerWidth, currentNotePosition);
    ...
    get tempo() {
        return this._player.desiredQPM;
    }
...
class NoteSequenceDrawing {
    constructor(sequence, canvas) {
        this.config = {
            noteHeight: 8,
            noteSpacing: 1,
            pixelsPerTimeStep: 40,  // The bigger this number the "wider" a note looks,
            minPitch: 100,
            maxPitch: 1
        };
    ...
    drawSequence(currentNote) {
        ...
        for (let i = 0; i < this.noteSequence.notes.length; i++) {
            ...
            if (NoteSequenceDrawing.isPaintingCurrentNote(note, currentNote)) {
                this.ctx.fillStyle = `rgba(184, 52, 34, ${opacity})`;
            } else if (i <= this.primerNotes) {
                this.ctx.fillStyle = `rgba(55, 124, 76, ${opacity})`;
            }
            else {
                this.ctx.fillStyle = `rgba(73, 161, 111, ${opacity})`;
            }
    ...
    static isPaintingCurrentNote(note, currentNote) {
        return currentNote &&
